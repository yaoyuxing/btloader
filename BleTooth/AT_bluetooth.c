...
#define    CMD_SET_STR_COMP_LEN      (9)
 stCmdCtrlType  gstCmdCtrl ={{ 
	                      AtBlueModeCommand,BL_MODE,{COMMAND_MODE_RETURN,PENETRATE_MODE_RETURN},200, //初始化命令控制结构
...
void AtBlueModeCommand(char *name ,char* return_command,eBlCommandMode CommandMode)
{
	char* p_return =return_command;
	switch(CommandMode)
	{
		case   eBlCommandInquire://查询
			p_return=strcat (p_return,BL_MODE); 
			break;
